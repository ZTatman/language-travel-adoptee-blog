...
export default function BlogCard({ post }: Props) {
  return (
    <Card key={post._id} className="group mx-auto my-0 h-full w-full max-w-md cursor-pointer overflow-hidden shadow-lg">
      ...
      <CardContent className="space-y-3 p-4">
        <CardTitle className="font-heading text-xl line-clamp-1 group-hover:text-sky-500">{post.title}</CardTitle>
        <CardDescription className="font-sans text-sm line-clamp-2">{post.description}</CardDescription>
      </CardContent>
      <CardFooter className="flex flex-row items-center space-x-4 px-4 py-2 font-sans text-xs text-gray-600">
        {/* Author */}
