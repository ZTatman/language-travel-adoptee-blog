...
export function Button({ onClick, rounded = false, filled = false, children = null }: PropsWithChildren<ButtonProps>) {
  return (
    <ButtonBase
      className={`text-sm ${rounded ? "rounded-full" : ""} ${
        filled
      ...
      } px-6 py-2 leading-none transition duration-150 ease-in-out sm:inline-block`}
      onClick={onClick}
    >
      {children}
    </ButtonBase>
